...
<template>
  <div>
    <svg
      @click="openMenu()"
      class="w-5 h-5 flex-none text-white"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    ...
    >
      ...
      <Link
        ...
        :class="
          route().current('dashboard')
            ? 'border-c-green-100 bg-slate-600 text-white'
      ...
      <div class="relative">
        <div
          ...
          class="flex items-center justify-between px-6 py-2 text-slate-300 text-sm cursor-pointer"
        >
          <span>Settings</span>
          <svg
            class="w-[18px] h-[18px] transform transition duration-250"
        ...
        <div v-if="showSettings">
          <Link
            ...
            :class="
              route().current('settings.profile.edit')
                ? 'border-c-green-100 bg-slate-600 text-white'
                : 'border-transparent text-slate-300'
            "
          ...
          <Link
            class="flex items-center space-x-2 text-sm border-r-4 pl-8 py-2 transition"
            :class="
              route().current('settings.password.edit')
                ? 'border-c-green-100 bg-slate-600 text-white'
                : 'border-transparent text-slate-300'
            "
