...
const form = useForm({
  name: page.props.value.product.name,
  description: page.props.value.product.description,
  price: page.props.value.product.price,
  section_id: page.props.value.product.section_id,
  category_id: page.props.value.product.category_id,
  image: null,
});
...
const handleFileUpload = async (event) => {
  ...
  if (file) {
    // Update the form with the file and its name
    form.image = file;
...
const updateProduct = () => {
  const productId = page.props.value.product.id;

  // Submit all fields for validation
  Inertia.post(route("product.post.update", productId), form, {
    onStart: () => {
...
<template>
  <DashboardLayout>
    ...
    <header class="mb-8">
      <h2 class="font-medium uppercase text-2xl">Edit Product</h2>
    </header>
    ...
    <form @submit.prevent="updateProduct()">
      ...
      <div class="mb-5">
        ...
        <Dropdown>
          <template #trigger>
            <button  type="button" class="mt-1 w-full placeholder-zinc-400 py-2 px-3 text-left border rounded text-gray-700 focus:outline-none">
              {{ categories.find((cat) => cat.id === form.category_id)?.name || "Select a Category" }}
            </button>
          </template>
          <template #content>
      ...
      <div class="mb-5">
        ...
        <Dropdown>
          <template #trigger>
            <button  type="button" class="mt-1 w-full placeholder-zinc-400 py-2 px-3 text-left border rounded text-gray-700 focus:outline-none">
              {{ sections.find((sec) => sec.id === form.section_id)?.name || "Select a Section" }}
            </button>
          </template>
          <template #content>
            <div class="max-h-60 overflow-y-auto">
